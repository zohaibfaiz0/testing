...
import { fetchStuffProducts } from '@/sanity/lib/sanityClient';
...
export default function ProductList() {
  ...
  useEffect(() => {
    const getProducts = async () => {
      try {
        setIsLoading(true);
        const fetchedProducts = await fetchStuffProducts();
  ...
  if (error) {
    return <div className="text-center py-10 text-red-600">{error}</div>; // Display error message
  }
  ...
  return (
    <div className="container mx-auto px-4">
      <h2 className="text-center text-2xl sm:text-3xl mt-6 md:text-4xl font-bold text-gray-800 mb-8 uppercase tracking-wider">
        <span className='text-red-600'>━ </span>Accessories<span className='text-red-600'> ━</span>
      </h2>
      <div className="flex flex-wrap gap-4 p-4 w-full justify-center">
        {products.map((product) => (
          ...
          >
            <Link href={`/product/${product._id}`} className="h-full flex flex-col" aria-label={`View details for ${product.name}`}>
